...
class Camera
{
    constructor( serial, devicePath, httpPort, configuration )
    {
        // Camera properties
        this.serial     = serial;
        this.devicePath = devicePath;
        this.httpPort   = httpPort;
        this.wsPort     = httpPort-100;
        this.alive      = true;
        ...
        this.listeners =
        {
            registration: new Listener( this.eventBus, "broadcastRegistration", false, () =>
            {
                if( this.alive === true )
                {
                    ...
                    {
                        port:		        ( this.serial === 'pilot' ? this.wsPort : this.httpPort ),
        ...
        this.client.on('connect', () => {
            ...
            this.client.subscribe(`video/${this.serial}/settings`);
        });
        ...
        this.client.on('message', (topic, payload) => {
          if (topic.match(`video/${this.serial}/settings`) !== null) {
            log(`MJPEG-VIDEO-SERVER: Received video/${this.serial}/settings MQTT message ${payload}`);
            let obj = JSON.parse(payload);
            this.eventBus.emit('updateSettings', obj);
          }
        })
    };
        ...
        else if ( process.env.EXTERNAL_CAM )
        {
            ...
            if (this.settings.record && this.serial === 'pilot')
            {
                return [
                    ...
                    "-o", `output_http.so -p 7011 -w /opt/openrov/www-ro`,
                    "-o", `output_http.so -p ${this.httpPort} -w /opt/openrov/www`,
                    "-o", `output_ws.so -p ${this.wsPort}`,
