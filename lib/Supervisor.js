...
const ExternalCamera    = require( "ExternalCamera" );
const maxCameras    = 1;
...
class Supervisor
{
    ...
    scanForCameras()
    {
        ...
        else
        {
            ...
            return Promise.try( () =>
            {
                if(process.env.EXTERNAL_CAM === 'true') {
                    // Create the external camera

                    log( `Creating External Camera` );
                    return Promise.try( () =>
                        {
                        ...
                        .catch( (err) =>
                        {
                            error( `Error creating External Camera: ${err.message}` );
                        })
                }
                else if( this.settings.useMock === true && this.settings.useHardware === false )
                {
    ...
    createExternalCamera()
    {
        ...
        {
            // Create the camera
            this.cameras[ serial ] = new ExternalCamera( serial, port, 
            {
                sioServer:          this.sioServer,
                eventBus:           this.eventBus
            } );
        ...
        {
            throw new Error( "Could not create External Camera. Limit reached." );
        }
    }
