...
const maxCameras    = process.env.MAX_CAMERAS || 1;
...
class Supervisor
{
    ...
    scanForCameras()
    {
        ...
        else
        {
            ...
            return Promise.try( () =>
            {
                ...
                // if this is set, assume all cameras are IP cameras
                {
                    if( process.env.EXTERNAL_CAM_URL === undefined ) {
                        throw new Error( "EXTERNAL_CAM_URL is not defined" );
                    }
                    return Promise.try( () =>
                    {
                        log( `Creating external networked MJPEG camera` );
                        this.createCamera('pilot', process.env.EXTERNAL_CAM_URL);
                    })
                    .then( () => {
                        // now create other external cameras if specified
                        if ( process.env.hasOwnProperty('OTHER_CAM_URL') )
                        {
                            // should be list of space-separated URLs in the form
                            // http://<IP>:8081/bmimg
                            let u = process.env.OTHER_CAM_URL.split(' ');
                            return Promise.map(u, ( devicePath ) => {
                                // use IP last octet for camera 'serial' id
                                let ser = devicePath.split(':',2)[1].split('.')[3]
                                log( `Creating external networked MJPEG camera` );
                                return Promise.try( () =>
                                {
                                    this.createCamera(ser, devicePath);
                                })
                                .catch( (err) =>
                                {
    ...
    createCamera( serial, devicePath )
    {
        ...
        {
            throw new Error( "Camera already exists at " + devicePath );
        }
    ...
    removeCamera( serial )
    {
        ...
        return this.cameras[ serial ].stop()
            ...
            {
                // Recycle port number
                this.portPool.recycle( this.cameras[ serial ].httpPort );
